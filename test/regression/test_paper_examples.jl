...
@testset "Section 4.2: sexually transmitted infection with vertical transmission" begin
    ...
    # maturation of infected juveniles is a transition
    ...
    for _ in 1:5
        ...
        @test ReproductiveNumbers.to_number(substitute(R0, vals)) ≈ basic_reproduction_number(num)
        @test validate_decomposition(num)
        @test sign(basic_reproduction_number(num) - 1) == sign(maximum(real, eigvals(num.T + num.Σ)))
    end
end
...
@testset "Section 4.3: bovine viral diarrhoea with persistently infected animals" begin
    ...
    for _ in 1:5
        ...
        vals[p₁] = rand(rng); vals[p₂] = rand(rng); vals[a] = vals[μ] * rand(rng)
        num = evaluate(ngm, vals)
        @test ReproductiveNumbers.to_number(substitute(R0, vals)) ≈ basic_reproduction_number(num)
        @test validate_decomposition(num)
        @test sign(basic_reproduction_number(num) - 1) == sign(maximum(real, eigvals(num.T + num.Σ)))
...
@testset "Staged progression (van den Driessche & Watmough 2002, section 4.3)" begin
    ...
    paper = sum(βv[k] * prod(νv[j] for j in 1:(k - 1); init = 1) / prod(νv[j] + d for j in 1:k) for k in 1:n)
